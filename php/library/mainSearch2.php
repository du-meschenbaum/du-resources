<?php
	...
	if (isset($_POST['book_search']) && isset($_POST['book_keyword']) && $_POST['book_keyword'] != "") {
		...
		$catalog_keyword = str_replace(
 				array("\xe2\x80\x98", "\xe2\x80\x99", "\xe2\x80\
x9c", "\xe2\x80\x9d", "\xe2\x80\x93", "\xe2\x80\x94", "\xe2\x80\xa6", '"'),
 				array("'", "'", '%22', '%22', '-', '--', '...', '%22'),
 				$catalog_keyword);
...
case "summonAll" :
			$location = "http://du.summon.serialssolutions.com/search?s.q=" . $catalog_keyword . "&s.fvf[]=ContentType%2CNewspaper+Article%2Ct" . "";
			header("Location: $location");
			break;
